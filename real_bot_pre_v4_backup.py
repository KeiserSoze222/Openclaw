...
assert MARKET_SERIES == "KXBTC15M", "WRONG MARKET in real_bot.py"
...
def get_max_bet():
    """SOL: Stop completely during bad hours where WR is 0-40%."""
    base = max(2.00, min(20.00, round(CURRENT_BALANCE * MAX_BET_PCT, 2)))
    hour = datetime.datetime.now(datetime.timezone.utc).hour
    # SOL bad hours — 0-40% WR — stop completely
    if hour in (0, 1, 4, 5, 12, 13, 22, 23):
...
def log_trade(direction, bet, action, profit_pct=0, notes=""):
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open("sol_trade_log.csv", "a", newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            timestamp, direction, f"{bet:.2f}",
            action, f"{profit_pct:.2f}%", notes
        ])
    # Also write to JSON performance log
    import json
    entry = {
        "timestamp": timestamp,
        "direction": direction,
    ...
    log_path = "/root/sol_performance_log.json"
    try:
...
def place_order(direction, bet, strategy_tag="directional", mkt_yes=None, mkt_no=None):
    # TOD check — but always allow extreme signals (YES < 0.20 or YES > 0.80)
    is_extreme = mkt_yes is not None and (mkt_yes < 0.20 or mkt_yes > 0.80)
    if get_max_bet() == 0.00 and not is_extreme:
        print(f"[TOD] Skipping trade — bad hour for this bot")
        return False
    if get_max_bet() == 0.00 and is_extreme:
        bet = max(2.00, round(CURRENT_BALANCE * 0.03, 2))  # 3% for extreme during bad hours
    """
...
def try_directional(yes_price, no_price):
    ...
    # STRONG first signal: edge > 0.40 in minute 1 with Coinbase confirmation
    elif window_minute == 1 and edge >= 0.40:
        spot = get_binance_price_for_market(MARKET_SERIES)
        if spot is not None:
            try:
                ...
                if mresp.status_code == 200:
                    ...
                    if strike > 0:
                        ...
                        coinbase_confirms = (current_direction == "UP" and spot_gap > 0) or                                            (current_direction == "DOWN" and spot_gap < 0)
                        if coinbase_confirms:
                            print(f"[Signal] ⚡ STRONG MIN1 {current_direction} | yes={yes_price:.2f} | "
                                  f"edge={edge:.2f} | Coinbase confirms | firing early")
                            consecutive_signal_count = 2  # bypass confirmation
                        else:
                            print(f"[Signal] MIN1 strong edge but Coinbase disagrees — waiting for confirmation")
            except Exception:
                pass
        ...
        try:
            ...
            if btc_p and eth_p and sol_p:
                ...
                if btc_p2 and eth_p2 and sol_p2:
                    ...
                    if agreement >= 3:
                        bet = min(round(bet * 1.20, 2), get_max_bet())
                        print(f"[Correlation] All 3 markets agree {current_direction} — bet boosted to ${bet:.2f}")
                    elif agreement == 1:
                        bet = round(bet * 0.80, 2)
                        print(f"[Correlation] Markets diverging — bet reduced to ${bet:.2f}")
                    else:
                        print(f"[Correlation] 2/3 markets agree {current_direction}")
        except Exception:
            pass
                    ...
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            if result in ("yes", "no"):
                                ...
                                log_trade(direction, bet, outcome, profit_pct=pnl/bet*100, notes=f"{strategy}|{ticker}")
                                to_remove.append(pos)
                    except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*50}")
    print(f"OpenClaw BTC Bot v3.0 — {mode}")
    print(f"Balance: ${CURRENT_BALANCE:.2f} | "
