#!/usr/bin/env python3
"""OpenClaw Arena v1.0 — Self-Adjusting Tournament Engine"""
...
if MARKET_SERIES == "KXBTC15M":
    ...
    TOD_SCHEDULE={0:0.00,1:0.00,2:0.00,3:0.00,4:0.00,5:0.00,6:1.00,7:1.00,8:1.00,9:1.00,10:1.00,11:1.00,12:0.50,13:0.50,14:1.00,15:1.00,16:1.00,17:0.75,18:0.75,19:1.00,20:0.75,21:1.00,22:0.00,23:0.00}
...
def try_directional(yes_price,no_price):
    ...
    write_arena_signal(market=_args.market,yes_prob=yes_price,regime=REGIME,confidence=0,window_age=window_minute,ticker=live_ticker)
...
def load_json(p,d):
    try: return __import__("json").load(open(p)) if os.path.exists(p) else d
    except: return d
def save_json(p,d):
    __import__("json").dump(d,open(p,"w"),indent=2,default=str)
def get_genome_pool():
    pool=load_json(GENOME_FILE,[])
    if pool: return pool
    markets=["btc","btc","btc","btc","btc","eth","eth","eth","sol","sol","btc","btc"]
    thresholds=[0.70,0.65,0.75,0.68,0.70,0.70,0.65,0.75,0.70,0.65,0.70,0.70]
    bets=[0.07,0.09,0.05,0.07,0.07,0.09,0.11,0.07,0.05,0.07,0.07,0.07]
    confs=[6,5,7,6,6,6,5,7,6,5,6,6]
    pool=[{"id":f"v{i+1:03d}","market":markets[i],"entry_threshold":thresholds[i],"max_bet_pct":bets[i],"min_confidence":confs[i],"chop_allowed":i==3,"news_weight":0.3,"wins":0,"losses":0,"trades":0,"pnl":0.0,"generation":1,"status":"active","mutations":[]} for i in range(12)]
...
if __name__=="__main__":
    ...
    pool=get_genome_pool()
    print(f"[Arena] Loaded {len(pool)} variants")
    cycle=0
    while True:
        cycle+=1
        pool=load_json(GENOME_FILE,pool)
        for i,v in enumerate(pool):
            if v.get("status")!="active": continue
            mkt=v.get("market","btc")
            sig=load_json(f"/root/arena_signal_{mkt}.json",{})
            if not sig: continue
            yp=sig.get("yes_prob",0.5)
            regime=sig.get("regime","CHOP")
            conf=sig.get("confidence",0)
            age=sig.get("window_age",99)
            if regime=="CHOP" and not v.get("chop_allowed"): continue
            if age>13: continue
            if conf<0: continue
            thresh=v.get("entry_threshold",0.70)
            if yp>=thresh: direction="UP"
            elif (1-yp)>=thresh: direction="DOWN"
            else: continue
            win=random.random()<(yp if direction=="UP" else 1-yp)
            outcome="WIN" if win else "LOSS"
            bet=round(v["max_bet_pct"]*CURRENT_BALANCE,2)
            v["trades"]=v.get("trades",0)+1
            v["wins"]=v.get("wins",0)+(1 if win else 0)
            v["losses"]=v.get("losses",0)+(0 if win else 1)
            v["pnl"]=round(v.get("pnl",0)+(bet*0.85 if win else -bet),2)
            wr=v["wins"]/v["trades"] if v["trades"]>0 else 0
            print("[Arena] "+v["id"]+" "+direction+" "+outcome+" WR="+str(round(wr,2))+" T="+str(v["trades"])+" PnL="+str(v["pnl"]))
            result=evaluate(v)
